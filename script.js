...
function registerSW(){
  ...
  navigator.serviceWorker.register('/DesafioHV7/service-worker.js').then(reg=>{
    ...
    accountHTML = `
      <div class="config-user-card">
        ...
        <div class="config-user-info">
          <div class="config-user-name">${currentUser.displayName || 'Usuario'}</div>
...
function renderFriendsView() {
  ...
  const idCard = `<div class="profile-id-card">
    <div class="profile-id-info">
      <div class="profile-id-username">@${myProfile.username}</div>
      <div class="profile-id-code">Código: ${myProfile.friendCode}</div>
    </div>
    <button class="profile-id-copy-btn" onclick="copyFriendCode()">📋 Copiar código</button>
  </div>`;

  // ── Solicitudes pendientes ──
  let requestsHTML = '';
  if (friendRequestsCache.length) {
    requestsHTML = `<div class="friend-requests-banner">
      <div class="friend-requests-title">🔔 Solicitudes de amistad (${friendRequestsCache.length})</div>
      ${friendRequestsCache.map(r => `<div class="friend-request-item">
        <div class="fr-name">@${r.fromUsername}</div>
        <div class="fr-actions">
          <button class="fr-accept-btn" onclick="doAcceptFriendRequest('${r.fromUid}','${r.fromUsername}')">Aceptar</button>
          <button class="fr-reject-btn" onclick="doRejectFriendRequest('${r.fromUid}')">Rechazar</button>
        </div>
      </div>`).join('')}
    </div>`;
  }
  ...
  // ── Botones de acción ──
  const actionsHTML = `<div class="friends-add-row">
    <button class="friends-add-btn" onclick="openAddFriendModal()">➕ Agregar amigo</button>
    <button class="friends-add-btn secondary" onclick="openChallengeModal()">⚔️ Nuevo desafío</button>
  </div>`;

  // ── Desafíos activos / pendientes / terminados ──
  ...
  function renderChallengeCard(c) {
    ...
    const myName = isFromSide ? c.fromUsername : c.toUsername;
    ...
    if (c.status === 'active' && c.endsAtMs) {
      ...
      metaHTML = `<div class="challenge-meta"><span>${daysLeft} día${daysLeft!==1?'s':''} restante${daysLeft!==1?'s':''}</span><span>${c.durationDays} días en total</span></div>`;
    } else if (c.status === 'pending') {
      metaHTML = `<div class="challenge-meta"><span>Duración propuesta: ${c.durationDays} días</span></div>`;
    }

    let actionsHTML2 = '';
    if (isIncoming) {
      actionsHTML2 = `<div class="challenge-actions">
        <button class="challenge-accept-btn" onclick="doAcceptChallenge('${c.id}',${c.durationDays})">Aceptar</button>
        <button class="challenge-decline-btn" onclick="doDeclineChallenge('${c.id}')">Rechazar</button>
      </div>`;
    } else if (isOutgoingPending) {
      actionsHTML2 = `<div class="challenge-meta"><span>⏳ Esperando respuesta de @${theirName}</span></div>`;
    }

    let winnerHTML = '';
    if (c.status === 'finished') {
      if (!c.winnerUid) winnerHTML = `<div class="challenge-winner-banner lose">🤝 Empate</div>`;
      else if (c.winnerUid === currentUser.uid) winnerHTML = `<div class="challenge-winner-banner win">🏆 ¡Ganaste!</div>`;
      else winnerHTML = `<div class="challenge-winner-banner lose">😔 Ganó @${theirName}</div>`;
    } else if (c.status === 'declined') {
      winnerHTML = `<div class="challenge-winner-banner lose">Desafío rechazado</div>`;
    }

    const canDelete = c.status === 'finished' || c.status === 'declined' || isOutgoingPending;

    return `<div class="challenge-card ${c.status}">
      ${canDelete ? `<button class="challenge-delete-btn" onclick="doDeleteChallenge('${c.id}')">🗑</button>` : ''}
      <div class="challenge-top">
        <span class="challenge-icon">${t.icon}</span>
        <span class="challenge-type-label">${t.label}</span>
        <span class="challenge-status-badge ${c.status}">${({pending:'Pendiente',active:'En curso',finished:'Finalizado',declined:'Rechazado'})[c.status]||c.status}</span>
      </div>
      <div class="challenge-vs-row">
        <div class="challenge-side"><div class="challenge-side-name">Vos</div><div class="challenge-side-val">${myVal}</div></div>
        <div class="challenge-vs-divider">VS</div>
        <div class="challenge-side"><div class="challenge-side-name">@${theirName}</div><div class="challenge-side-val">${theirVal}</div></div>
      </div>
      ${c.status==='active'?`<div class="challenge-bar-bg"><div class="challenge-bar-mine" style="width:${myPct}%"></div><div class="challenge-bar-theirs" style="width:${theirPct}%"></div></div>`:''}
      ${metaHTML}
      ${actionsHTML2}
      ${winnerHTML}
    </div>`;
  ...
  let challengesHTML = '';
  if (pending.length) challengesHTML += `<div class="sec-subtitle">Pendientes (${pending.length})</div>` + pending.map(renderChallengeCard).join('');
  if (active.length) challengesHTML += `<div class="sec-subtitle">En curso (${active.length})</div>` + active.map(renderChallengeCard).join('');
  if (finished.length) challengesHTML += `<div class="sec-subtitle">Historial</div>` + finished.slice(0,10).map(renderChallengeCard).join('');
  if (!challengesHTML) challengesHTML = `<div class="empty-friends">Todavía no tenés desafíos. ¡Creá uno! ⚔️</div>`;

  // ── Lista de amigos ──
  let friendsHTML2 = `<div class="sec-subtitle">Tus amigos (${friendsListCache.length})</div>`;
  if (!friendsListCache.length) {
    friendsHTML2 += `<div class="empty-friends">Todavía no agregaste amigos 👥</div>`;
  } else {
    friendsHTML2 += friendsListCache.map(f => `<div class="friend-card">
      <div class="friend-avatar">${(f.username||'U')[0].toUpperCase()}</div>
      <div class="friend-info"><div class="friend-name">@${f.username}</div><div class="friend-stars">⭐ ${f.totalStars}</div></div>
      <div class="friend-actions">
        <button class="friend-challenge-btn" onclick="openChallengeModal('${f.uid}')">⚔️ Desafiar</button>
        <button class="friend-remove-btn" onclick="doRemoveFriend('${f.uid}','${f.username}')">✕</button>
      </div>
    </div>`).join('');
  }
  ...
  container.innerHTML = `
    <div class="friends-header">
      <div class="friends-title">👥 Amigos & Desafíos</div>
      <div class="friends-sub">Competí con tus amigos y mantené la motivación</div>
    </div>
    ${idCard}
    ${requestsHTML}
    ${actionsHTML}
    <div class="sec-subtitle">Desafíos</div>
    ${challengesHTML}
    ${friendsHTML2}
  `;
}
...
async function setupSocialFeatures() {
  ...
  if (myProfile && myProfile.username) {
    ...
    unsubFriendRequests = listenFriendRequests(currentUser.uid, (reqs) => {
      ...
      if (document.getElementById('friends-view').style.display !== 'none') renderFriendsView();
    });
